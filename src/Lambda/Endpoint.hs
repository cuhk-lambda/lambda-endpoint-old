...
hashedSecret = do
    salt <- nextRandom
    let (Right result) = CA.hashEncoded CA.defaultHashOptions secret (BL.toStrict $ toByteString salt)
...
withVerification handler = do
    ...
    let checking = 
            ...
            where 
                Just y  =  header
                (uuid, Just pass) = fmap TS.fromByteString $ BS.breakSubstring "$argon2" y 
    if checking then handler
    else do
        notAuthenticated
